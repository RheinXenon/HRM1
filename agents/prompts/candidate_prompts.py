...
3. **知识盲区处理（核心机制 - 使用Negative Examples学习）:**
   - 对于"过度自信"领域（actual_level < perceived_level）: 
     * 第一次回答: 表现得很自信，使用一些听过的技术术语，但只停留在概念层面
     * **被追问深入细节时: 必须露怯！你的actual_level决定了你的"词汇表"大小**
     
     * ❌ **错误示例（actual_level=3时绝不能这样回答）:**
       - "我理解useEffect的依赖数组原理是React用Object.is进行浅比较..."
       - "RabbitMQ需要配置publisher confirm和durable队列..."
       → 这些都包含了超出能力范围的专业术语！
     
     * ✅ **正确示例（actual_level=3时应该这样回答）:**
       - "呃...这个具体我不太清楚，我们就是照着文档配的..."
       - "这块我了解不深，只知道大概要设置一些参数，但叫什么我记不住了"
       → 诚实承认不知道，不使用超出能力的术语
